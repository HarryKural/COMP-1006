<?php
  // Step 1 - Create the following PHP variables: Band Name, Band Founded Date (using the PHP date function).
  $bandName = "The Beatles <small>Rock Band</small>";
  $bandFoundedDate = date(1960);

  // Step 2 - Create the following PHP array: Band Members
  $bandMembers = array(
                      "John Lennon", "Paul McCartney", "Ringo Starr",
                      "George Harrison", "Pete Best", "Stuart Sutcliffe"
  );
?>

<!DOCTYPE HTML>
<html lang="en">

  <head>
    <link rel="stylesheet" type="text/css" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.5.0/css/font-awesome.min.css">
    <link rel="stylesheet" type="text/css" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.6/css/bootstrap.min.css">
    <title>Lesson 01 - Lab</title>
  </head>

  <body>
    <div class="container">

      <header>
        <h1 class="page-header">Lesson 01 Lab<small>&nbsp;&mdash;&nbsp;Dynamic Pages</small></h1>
      </header>

      <section>
        <!-- Step 3 - Fill in the placeholders -->
        <div class="panel panel-default">
          <div class="panel-heading">
            <h3 class="panel-title"><?php echo $bandName; ?><!-- Output PHP variable for Band Name (double check the data exists so the page doesn't throw an error) --></h3>
          </div>

          <div class="panel-body">
            <div>
              <h3>Band Members</h3>
              <p>
                <!-- Output PHP array for Band Members using a foreach loop -->
                <?php
                  foreach ($bandMembers as $bandMember)
                    {
                      echo "<li>$bandMember </li><br>";
                    }
                ?>
              </p>
              <h3>Band Founded Date</h3>
              <p>
                <!-- Output PHP variable for Band Founded Date -->
                <?php echo $bandFoundedDate; ?>
              </p>
            </div>
          </div>
        </div>
      </section>

      <!-- Step 3 - Upload to your web server and submit your link on Blackboard for 5 marks -->
      <!-- DONE -->
    </div>

    <script type="text/javascript" src="https://code.jquery.com/jquery-2.2.0.min.js"></script>
    <script type="text/javascript" src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.6/js/bootstrap.min.js"></script>
  </body>
  
</html>
